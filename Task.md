...
- Add a Jinja-callable helper (e.g. a `avatar_url(user_id)` template global, or checking file existence in each route and passing an `avatar_exists`/`avatar_url` value into the template context) usable across profile/index/thread without duplicating the file-existence check in every route.
- Add the avatar `<img>` (or placeholder circle) to `profile.html`'s header, `index.html`'s per-post meta line, and `thread.html`'s per-message meta line.
- Style the avatar (small circle, consistent size in each context) and the placeholder fallback.
- Verify with Playwright: a freshly signed-up user's avatar renders correctly-cropped on their own profile page and next to their posts in the stream; an account with no avatar file (the seeded bot) shows the placeholder instead of a broken image.
