...
- Add `user_facts` table to schema.sql.
- Add a small `extract_facts(body)` function: regex/keyword rules for city, offers, needs, channel: returns a list of (key, value) pairs found.
- Wire it into `thread()`'s POST handler: only when the thread `is_bot_thread` and the sender is the human (not the bot), run extraction and insert any facts found tagged with `source_thread_id`.
- Verify with a scripted test-client run: a message like "I have a couch to give away, I'm in Portland, reach me at foo@example.com" sent in a bot thread produces offers/city/channel facts in `user_facts`; the same message sent in a human-to-human thread produces no facts at all; a message with no recognizable shape produces no facts (no false positives from ordinary chat).
