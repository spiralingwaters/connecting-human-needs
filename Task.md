...
- Add the welcome-message send to `/signup`'s POST success path: find-or-create a thread between the new user and the circulator bot, insert one message with the fixed welcome text.
- Guard for no bot persona existing (defensive, shouldn't trigger with the seed in place).
- Verify with a scripted test-client run: signing up a new user creates a bot thread from circulator with the welcome text already in it, visible in the new user's `/messages` Bots section and thread view.
